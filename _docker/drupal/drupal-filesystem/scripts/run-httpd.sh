...
#rm -rf /opt/rh/httpd24/root/etc/httpd/run/*

# Launch Apache as a child-process of this script so we can 'wait' on it
httpd -D FOREGROUND &

# Wait for the Apache pid file to be populated
until [ -e '/opt/rh/httpd24/root/etc/httpd/run/httpd.pid' ]; do
  sleep 5
  echo "Waiting for Drupal PID file at '/opt/rh/httpd24/root/etc/httpd/run/httpd.pid'"
done

# Read the pid that Apache is running as
PID=$(cat /opt/rh/httpd24/root/etc/httpd/run/httpd.pid)
